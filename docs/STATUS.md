...
Stanje na dan **25. 9. 2026**, posle dela 7a — tranziti su gotovi, sledi 7b (uplate). Ovaj dokument je polazna tačka za svaku novu radnu sesiju: šta je gotovo, gde se šta nalazi, šta je odlučeno i šta sledi.
...
| 7a | `fdf4bea`, „Phase 7a: transits“ | Tranziti prema natalnoj karti klijenta i povezane osobe (tab „Transits“: izbor trenutka i „Now“, dvostruki točak, kontakti po orbu sa datumima kada su spore planete tačne, tranzitne pozicije u natalnim kućama), zaseban skup orba za tranzite (Settings → Chart & methods), Hiron u karti i tranzitima, „Transits for this date“ iz termina i „Transits on this date“ sa konsultacije, kartica „Before your next consultations“ na dashboardu |

Testovi posle Faze 7a: **310 PHP** (2397 provera; od toga referentni testovi pozicija prema NASA JPL Horizons, i za Hiron, i 29 testova uglova i kuća prema nezavisnim formulama — oba skupa se izvršavaju samo gde postoji `swetest`) i **109 Vitest**; CI: GitHub Actions (Pint, Prettier, Vitest, build, PHPUnit na MariaDB 11.8).
...
11. Odloženo iz Faze 6c: podsetnik za zadatak i „Remind me“ iz prototipa (Faza 7, notifikacije); izbor odgovornog u interfejsu i filter „moji zadaci“ (timovi); fajlovi na zadatku (dokument 02 ih pominje; `attachments` je polimorfna, pa je to mala dopuna kad zatreba); broj zadataka u levom meniju; neplaćene konsultacije i prihod na dashboardu (Faza 7b).
12. Odloženo iz Faze 7a: „Sky calendar“ (`sky.html` u prototipu — ingresi, retrogradnosti, mene) i sinastrija nisu u dokumentu 04, ostaju za kasnije; tranziti za PDF (Faza 9); merenje niza od 731 dan na Linux-u (produkcioni server). Za validacione razgovore: da li su orbi za tranzite (2° / 1,5°) i pravilo za dashboard (Jupiter–Pluton, lične tačke, 1°) ono što astrolozi gledaju pred konsultaciju.
13. Lokalno: stara baza `astrolabe.online__10.2026` (sa tačkom) može da se obriše; test workspace je podešen na **sidereal/Lahiri i Whole Sign** (iz testa u Fazi 1) — menja se u Settings → Chart & methods. Tri test klijenta iz Faze 5 mogu da se arhiviraju.

## Sledeće

Faze 0–6 su završene (Faza 6 u tri dela: 6a, 6b, 6c). **Faza 7 — tranziti i finansije** ide u tri dela: **7a** tranziti (završeno), **7b** uplate i pokazatelji (sledeće), **7c** obaveštenja i podsetnici — svaki sa commit-om, zelenim CI-jem i tačkom za pauzu. Korisnik je odlučio da se ide odmah, pre validacionih razgovora.
