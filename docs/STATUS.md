...
4. **Produkcioni server** (Hetzner) i lokalni prelazak na MariaDB 11.8. Mail na produkciji ide preko lokalnog SMTP-a na serveru (odluka 25. 9. 2026) — tamo se proverava i slanje obaveštenja iz Faze 7c. Za upload na produkciji: PHP-FPM `upload_max_filesize` / `post_max_size` i `client_max_body_size` web servera moraju biti bar `ATTACHMENTS_MAX_MB`.
